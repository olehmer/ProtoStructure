...
def CalculateTopU(layer, R_gas):
    ...
    cur_loss = 4.0*pi*layer.rho*u*layer.r
...
def LayersInit(N, total_flux, core_mass, core_rad, atmos_mass, R_gas,\
    ...
    p_profile = np.logspace(np.log10(p_s),np.log10(p_toa), N) #log scale
...
def BalanceAtmosphere(core_mass, core_rad, atmos_mass, R_gas, F_uv, F_sol, F_long,\
        kappa_uv, kappa_sol, kappa_long, T_prof_in=[], N=100, iter_lim=200, \
        tol=1.0E-4, p_toa_in=-1.0):
    """
